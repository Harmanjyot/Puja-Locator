...
class quiz2: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        langquiz2 = langsetquiz
        langSet(langFAQ: langquiz2)
    ...
    @IBAction func button1(_ sender: Any) {
        ...
        score = score + 0
        performSegue(withIdentifier: "quiz2Segue", sender: nil)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "quiz2Segue"{
            guard let langquiz = segue.destination as? endQuiz1 else {return}
            langquiz.langsetquiz = langquiz2
